...
export default function GradeSimulator() {
  ...
  const currentCgpa = regulation && semestersWithSgpa.length > 0 
    ...
    : 0;
  const currentTotalCredits = semestersWithSgpa.reduce((sum, sem) => sum + (sem.totalCredits || 0), 0);

  const [simulatedSemesters, setSimulatedSemesters] = useState<{ id: string; name: string; sgpa: string; credits: string }[]>([
    { id: "1", name: `Semester ${semestersWithSgpa.length + 1}`, sgpa: "", credits: "" }
  ]);

  const addInput = () => {
    setSimulatedSemesters([
      ...simulatedSemesters, 
      { id: Date.now().toString(), name: `Semester ${semestersWithSgpa.length + simulatedSemesters.length + 1}`, sgpa: "", credits: "" }
    ]);
  };
  ...
  // Combine actual profile semesters with simulated ones
  ...
  const allSemesters = [
    ...semestersWithSgpa.map(sem => ({ sgpa: sem.sgpa as number, credits: sem.totalCredits || 0 })),
    ...validSimulatedSemesters
  ];

  const simulatedCgpa = regulation && allSemesters.length > 0 ? calculateCGPA(regulation, allSemesters) : currentCgpa;
      ...
      <div className="grid md:grid-cols-3 gap-8">
                ...
                <div key={input.id} className="flex flex-col sm:flex-row gap-4 p-4 rounded-xl border border-outline-variant/30 bg-surface-container-lowest transition-all hover:border-outline-variant">
                  ...
                  <div className="flex items-end pb-1 shrink-0">
                    <button 
                      ...
                      disabled={simulatedSemesters.length === 1}
        ...
        <div className="md:col-span-1 space-y-6">
          <div className="glass-panel rounded-2xl p-6 border-t-4 border-t-primary">
            ...
            <div className="space-y-6">
              <div className="flex items-center justify-between p-4 rounded-xl bg-surface-container-low border border-outline-variant/20">
                <span className="text-body-sm font-semibold text-outline">Current CGPA</span>
                <span className="text-title-md font-bold text-on-surface">{currentCgpa > 0 ? currentCgpa.toFixed(2) : "0.00"}</span>
              </div>
              ...
              <div className="p-4 rounded-xl bg-primary-container text-on-primary-container border border-primary/20 text-center relative overflow-hidden">
                ...
                <div className="flex items-baseline justify-center gap-2">
                  <span className="text-display-lg font-bold">{simulatedCgpa.toFixed(2)}</span>
                  <span className="text-title-lg opacity-70">/ {maxPoints}</span>
                </div>
                
                {validSimulatedSemesters.length > 0 && (
                  <div className={`mt-2 text-label-md font-bold ${isPositive ? 'text-primary' : 'text-error'}`}>
                    {isPositive ? '+' : ''}{diff} impact
                  </div>
                )}
              </div>
            </div>
          ...
          <div className="glass-panel rounded-2xl p-6 text-center">
             <div className="text-label-sm font-semibold text-outline uppercase tracking-wider mb-2">Total Credits Active</div>
             <p className="text-display-md text-on-surface font-bold">{currentTotalCredits + validSimulatedSemesters.reduce((sum, s) => sum + s.credits, 0)}</p>
          </div>
        </div>
      </div>
    </div>
