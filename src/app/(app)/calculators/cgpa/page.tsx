...
const defaultInput = { id: "1", name: "Semester 1", sgpa: "", credits: "" };
...
export default function CGPACalculator() {
  ...
  const validSemesters = inputs
    .map(input => {
      const sgpa = Number(input.sgpa);
      const credits = Number(input.credits);
      if (!input.sgpa || !input.credits || !Number.isFinite(sgpa) || !Number.isFinite(credits) || sgpa < 0 || sgpa > maxPoints || credits <= 0) {
        return null;
      }
      return { ...input, sgpa, credits };
    })
    .filter(Boolean) as { id: string; name: string; sgpa: number; credits: number }[];

  const currentCgpa = validSemesters.length > 0 ? calculateCGPA(regulation, validSemesters) : 0;
  const currentPercentage = calculatePercentage(regulation, currentCgpa, activeScale);
  ...
  const handleSaveToProfile = () => {
    if (validSemesters.length === 0) {
      alert("Enter at least one valid semester before saving.");
      return;
    }

    validSemesters.forEach((sem) => {
      addSemesterToStore({
        id: `cgpa-calc-${Date.now()}-${sem.id}`,
        name: sem.name.trim() || `Semester ${profile.semesters.length + 1}`,
        courses: [],
        sgpa: sem.sgpa,
        ...
        <div className="md:col-span-2 space-y-6">
          <div className="glass-panel rounded-2xl p-6">
              ...
              {inputs.map((input) => (
                <div key={input.id} className="flex flex-col sm:flex-row gap-4 p-4 rounded-xl border border-outline-variant/30 bg-surface-container-lowest transition-all hover:border-outline-variant">
                  ...
                  <div className="flex items-end pb-1 shrink-0">
                    <button
                      type="button"
                      aria-label={`Remove ${input.name || 'semester'}`}
            ...
            <div className="mt-6 flex justify-end">
              <button
                type="button"
                onClick={() => setInputs([{ ...defaultInput }])}
            ...
            {(hasSgpaError || hasCreditsError) && (
              <div className="mt-4 p-3 rounded-lg bg-error-container/50 border border-error/20 text-error text-label-sm flex flex-col gap-1">
                {hasSgpaError && <div className="flex items-center gap-2"><span className="font-bold">Error:</span> SGPA must be a number between 0 and {maxPoints}.</div>}
                {hasCreditsError && <div className="flex items-center gap-2"><span className="font-bold">Error:</span> Credits must be a valid positive number.</div>}
              </div>
            )}
          </div>
        </div>
        ...
        <div className="md:col-span-1">
          <div className="glass-panel rounded-2xl p-6 sticky top-24 border-t-4 border-t-primary">
            ...
            <button
              ...
              className="mt-8 w-full px-4 py-3 rounded-xl bg-primary text-on-primary font-semibold flex items-center justify-center gap-2 hover:bg-primary/90 transition-colors"
            >
              Save to Profile
            </button>
          </div>
