...
// Generic threshold generator for universities missing explicit mark requirements
function getGradeFromMarks(marks: number, gradingScale: any[]) {
  const sortedScale = [...gradingScale].sort((a, b) => b.points - a.points);
  
  if (marks >= 90) return sortedScale[0];
  if (marks >= 80) return sortedScale[Math.min(1, sortedScale.length - 1)];
  if (marks >= 70) return sortedScale[Math.min(2, sortedScale.length - 1)];
  if (marks >= 60) return sortedScale[Math.min(3, sortedScale.length - 1)];
  if (marks >= 50) return sortedScale[Math.min(4, sortedScale.length - 1)];
  if (marks >= 40) return sortedScale[Math.min(5, sortedScale.length - 1)];
  return sortedScale[sortedScale.length - 1]; // Fail grade usually
}

export function SeoMarksToCGPA({ university }: { university: University }) {
  ...
  const displayTitle = isGeneric ? "Marks to CGPA Calculator" : `${university.shortName} Marks to CGPA Calculator`;

  const [inputs, setInputs] = useState<{ id: string; subject: string; marks: string; credits: string }[]>([
    { id: "1", subject: "Subject 1", marks: "", credits: "3" },
    { id: "2", subject: "Subject 2", marks: "", credits: "3" },
    { id: "3", subject: "Subject 3", marks: "", credits: "4" }
  ]);

  const addInput = () => {
    setInputs([
      ...inputs, 
      { id: Date.now().toString(), subject: `Subject ${inputs.length + 1}`, marks: "", credits: "" }
    ]);
  };
  ...
  const handleUpdate = (id: string, field: "subject" | "marks" | "credits", value: string) => {
    setInputs(inputs.map(input => input.id === id ? { ...input, [field]: value } : input));
  };

  const validCourses = inputs.map(input => {
    if (input.marks === "" || input.credits === "") return null;
    const m = parseFloat(input.marks);
    const c = parseFloat(input.credits);
    if (isNaN(m) || isNaN(c) || c <= 0 || m < 0 || m > 100) return null;
    
    const gradeDef = getGradeFromMarks(m, activeScale.gradingScale);
  ...
  }).filter(Boolean) as { points: number, credits: number, grade: string }[];
  ...
  const hasMarksError = inputs.some(i => {
    const m = parseFloat(i.marks);
    return i.marks !== "" && !isNaN(m) && (m < 0 || m > 100);
  });
    ...
    <div className="space-y-12 animate-in fade-in slide-in-from-bottom-4 duration-500 max-w-5xl mx-auto">
      <div className="text-center space-y-4">
        ...
        <p className="text-body-lg text-on-surface-variant max-w-2xl mx-auto">
          Convert your subject marks (out of 100) into equivalent grades and calculate your {isGeneric ? 'SGPA and CGPA' : `${university.shortName} SGPA/CGPA`}.
        </p>
      </div>
        ...
        <div className="lg:col-span-2 space-y-6">
          <div className="glass-panel rounded-2xl p-6">
            <div className="flex items-center justify-between mb-6">
              <h3 className="text-title-md font-bold text-on-surface">Enter Subject Marks</h3>
            ...
            <div className="space-y-4">
              <div className="hidden sm:flex gap-4 px-4 text-label-sm text-outline font-semibold uppercase tracking-wider">
                ...
                <div className="flex-1">Marks (100)</div>
              ...
              {inputs.map((input) => (
                <div key={input.id} className="flex flex-col sm:flex-row gap-4 p-4 sm:p-2 sm:px-4 rounded-xl border border-outline-variant/30 sm:border-transparent bg-surface-container-lowest sm:bg-transparent transition-all hover:bg-surface-variant/20">
                  ...
                  <div className="flex-1">
                    <label className="block sm:hidden text-label-sm text-outline font-semibold mb-1 uppercase tracking-wider">Marks (out of 100)</label>
                    <input 
                      type="number"
                      step="1"
                      min="0"
                      max="100"
                      value={input.marks}
                      onChange={(e) => handleUpdate(input.id, "marks", e.target.value)}
                      placeholder="e.g. 85"
                      className="w-full h-11 px-3 rounded-lg bg-surface-container sm:bg-surface-container-low border border-outline-variant/30 focus:outline-none focus:border-primary/50 text-body-sm"
                    />
                  </div>
            ...
            <div className="mt-8 flex justify-end">
              <button 
                onClick={() => setInputs([{ id: "1", subject: "Subject 1", marks: "", credits: "" }])}
            ...
            {(hasMarksError || hasCreditsError) && (
              <div className="mt-4 p-3 rounded-lg bg-error-container/50 border border-error/20 text-error text-label-sm flex flex-col gap-1">
                {hasMarksError && <div className="flex items-center gap-2"><span className="font-bold">Error:</span> Marks must be between 0 and 100.</div>}
        ...
        <div className="lg:col-span-1 space-y-6">
          <div className="glass-panel rounded-2xl p-6 sticky top-24 border-t-4 border-t-primary">
            ...
            <div className="space-y-6">
              <div>
                <div className="text-label-md uppercase tracking-wider text-outline font-semibold mb-1">Estimated SGPA/CGPA</div>
                ...
                <div className="space-y-2">
                  {validCourses.map((c, i) => (
                    <div key={i} className="flex justify-between items-center text-body-sm">
                      <span className="text-outline-variant truncate mr-2">{inputs[i].subject || `Subject ${i+1}`}</span>
                      <div className="flex gap-2">
                        <span className="font-bold text-primary">{c.grade}</span>
                        <span className="text-outline">({c.points} pt)</span>
                      </div>
                    </div>
                  ))}
                </div>
            ...
            <div className="mt-4 text-xs text-outline-variant text-center">
              Note: This calculator uses standard Indian grade mapping (90+ = Top Grade). Actual official {university.shortName} mapping may vary slightly depending on the course.
            </div>
          </div>
        </div>
      </div>
      
      <DynamicFAQ university={university} pageType="marks-to-cgpa-calculator" />
    </div>
  );
}
