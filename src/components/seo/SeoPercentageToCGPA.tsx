...
import { calculateCgpaFromPercentage } from "@/lib/universities/engine";
import { Calculator, ArrowRight, Edit2, Check } from "lucide-react";
import { DynamicFAQ } from "./DynamicFAQ";

export function SeoPercentageToCGPA({ university }: { university: University }) {
  ...
  const displayTitle = isGeneric ? "Percentage to CGPA Calculator" : `${university.shortName} Percentage to CGPA`;
  ...
  const isValid = !isNaN(parsedPercentage) && parsedPercentage >= 0 && parsedPercentage <= 100;
  const hasError = percentageInput !== "" && (!isValid || parsedPercentage < 0 || parsedPercentage > 100);
  
  const cgpa = regulation && isValid ? calculateCgpaFromPercentage(regulation, parsedPercentage, activeScale, customFormula) : 0;
    ...
    <div className="space-y-12 animate-in fade-in slide-in-from-bottom-4 duration-500 max-w-4xl mx-auto">
      <div className="text-center">
        ...
        <p className="text-body-lg text-on-surface-variant mt-3 max-w-xl mx-auto flex flex-col sm:flex-row sm:items-center justify-center gap-1 sm:gap-4">
          <span>Convert your percentage back into a precise {isGeneric ? '' : university.name} CGPA.</span>
        </p>
      </div>

      <div className="glass-panel rounded-3xl p-8 sm:p-12 relative overflow-hidden">
        <div className="absolute top-0 left-0 w-full h-2 bg-gradient-to-r from-tertiary to-primary" />
        
        <div className="flex flex-col md:flex-row items-center gap-8 md:gap-12">
          
          <div className="flex-1 w-full text-center md:text-left">
            <label className="block text-label-lg font-bold text-outline uppercase tracking-widest mb-4">Your Percentage</label>
            <div className="relative">
              <input 
                ...
                className="w-full text-center md:text-left text-display-lg font-bold bg-transparent border-b-2 border-outline-variant/50 focus:border-primary focus:outline-none transition-colors pb-2 placeholder:text-outline-variant pr-12"
              />
              <span className="absolute right-2 bottom-4 text-title-lg text-outline font-bold">%</span>
            </div>
            {hasError && (
              <div className="mt-4 p-3 rounded-lg bg-error-container/50 border border-error/20 text-error text-label-sm text-left flex items-center gap-2">
                <span className="font-bold">Error:</span> Percentage must be between 0 and 100.
              </div>
            )}
          </div>
          ...
          <div className="flex-1 w-full flex flex-col items-center md:items-start gap-6 bg-surface-container-low p-6 rounded-2xl border border-outline-variant/20">
            <div>
              <div className="text-label-md text-outline font-semibold uppercase tracking-wider mb-2">Equivalent CGPA</div>
              <div className="flex items-baseline gap-2">
                <span className="text-display-lg font-bold text-primary">{isValid && cgpa > 0 ? cgpa.toFixed(2) : "0.00"}</span>
                <span className="text-title-lg text-on-surface-variant">/ {maxPoints}</span>
              </div>
            </div>
          </div>
        </div>
      ...
      <div className="p-4 rounded-xl bg-surface-variant/30 text-body-sm text-on-surface-variant text-center max-w-2xl mx-auto flex flex-col items-center gap-3">
        <div className="flex items-center gap-2">
          <strong>Engine Formula Solver:</strong> Automatically reversing 
          {!isEditingFormula ? (
        ...
        {isEditingFormula && (
          <div className="text-xs text-outline-variant">
            Use <code className="bg-surface-container px-1 rounded">cgpa</code> as the variable (e.g., <code className="bg-surface-container px-1 rounded">cgpa * 9.5</code>) to represent the target percentage formula.
          </div>
        )}
      </div>

      <DynamicFAQ university={university} pageType="percentage-to-cgpa-calculator" />
    </div>
  );
}
