...
export function SeoGPAToCGPA({ university }: { university: University }) {
  ...
  const displayTitle = isGeneric ? "GPA to CGPA Calculator" : `${university.shortName} GPA to CGPA Calculator`;

  const [inputs, setInputs] = useState<{ id: string; name: string; gpa: string; credits: string }[]>([
  ...
  const addInput = () => {
    setInputs([
      ...inputs, 
      { id: Date.now().toString(), name: `Semester ${inputs.length + 1}`, gpa: "", credits: "" }
    ]);
  };
  ...
  const handleUpdate = (id: string, field: "gpa" | "credits", value: string) => {
  ...
  const validSemesters = inputs
    .filter(i => {
      ...
      // Allow GPA inputs up to standard GPA scales (like 4.0 or matching maxPoints)
      return !isNaN(g) && !isNaN(c) && g >= 0 && g <= Math.max(maxPoints, 10) && c > 0;
    })
    // Map gpa -> sgpa internally for the engine
    .map(i => ({ sgpa: parseFloat(i.gpa), credits: parseFloat(i.credits) }));

  const currentCgpa = regulation && validSemesters.length > 0 ? calculateCGPA(regulation, validSemesters) : 0;

  const hasGpaError = inputs.some(i => {
    const g = parseFloat(i.gpa);
    return i.gpa !== "" && !isNaN(g) && (g < 0 || g > Math.max(maxPoints, 10));
  });
    ...
    <div className="space-y-12 animate-in fade-in slide-in-from-bottom-4 duration-500 max-w-4xl mx-auto">
      <div className="text-center space-y-4">
        ...
        <p className="text-body-lg text-on-surface-variant max-w-2xl mx-auto">
          Convert your semester GPA into a cumulative CGPA using {isGeneric ? 'standard weighted averaging' : `the official ${university.name} formula`}.
        </p>
      </div>
        ...
        <div className="md:col-span-2 space-y-6">
          <div className="glass-panel rounded-2xl p-6">
            <div className="flex items-center justify-between mb-6">
              <h3 className="text-title-md font-bold text-on-surface">Enter Semester GPAs</h3>
            ...
            <div className="space-y-4">
              {inputs.map((input) => (
                <div key={input.id} className="flex flex-col sm:flex-row gap-4 p-4 rounded-xl border border-outline-variant/30 bg-surface-container-lowest transition-all hover:border-outline-variant">
                  ...
                  <div className="flex-1">
                    <label className="block text-label-sm text-outline font-semibold mb-1 uppercase tracking-wider">GPA</label>
                    <input 
                      type="number" 
                      step="0.01"
                      min="0"
                      value={input.gpa}
                      onChange={(e) => handleUpdate(input.id, "gpa", e.target.value)}
            ...
            <div className="mt-6 flex justify-end">
              <button 
                onClick={() => setInputs([{ id: "1", name: "Semester 1", gpa: "", credits: "" }])}
            ...
            {(hasGpaError || hasCreditsError) && (
              <div className="mt-4 p-3 rounded-lg bg-error-container/50 border border-error/20 text-error text-label-sm flex flex-col gap-1">
                {hasGpaError && <div className="flex items-center gap-2"><span className="font-bold">Error:</span> GPA input seems invalid.</div>}
      ...
      <DynamicFAQ university={university} pageType="gpa-to-cgpa-calculator" />
    </div>
  );
}
