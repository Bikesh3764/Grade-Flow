...
export function SeoCGPACalculator({ university }: { university: University }) {
    ...
    <div className="space-y-12 animate-in fade-in slide-in-from-bottom-4 duration-500 max-w-4xl mx-auto">
      <div className="text-center space-y-4">
        <h1 className="text-display-sm md:text-display-md font-bold tracking-tight text-on-background">
          {university.shortName} CGPA Calculator
        </h1>
        <p className="text-body-lg text-on-surface-variant max-w-2xl mx-auto">
          Calculate your exact CGPA and percentage according to the official <strong className="text-primary">{university.name}</strong> grading regulations.
        </p>
      </div>
        ...
        <div className="md:col-span-2 space-y-6">
          <div className="glass-panel rounded-2xl p-6">
            <div className="flex items-center justify-between mb-6">
              <h3 className="text-title-md font-bold text-on-surface">Enter SGPA Details</h3>
            ...
            {(hasSgpaError || hasCreditsError) && (
              <div className="mt-4 p-3 rounded-lg bg-error-container/50 border border-error/20 text-error text-label-sm flex flex-col gap-1">
                {hasSgpaError && <div className="flex items-center gap-2"><span className="font-bold">Error:</span> SGPA cannot exceed {maxPoints} on the {university.shortName} scale.</div>}
      ...
      <DynamicFAQ university={university} pageType="cgpa-calculator" />
    </div>
  );
}
